...
class Space: # defines the spaces on a monopoly board
    ...
    community_chest = [1, 16, 32] # defines the spaces for community chest
    chance = [6, 21, 35] # defines the spaces for chance
    rent = [2, 0, 4, 0, 50, 6, 0, 6, 8, 0, 10, 0, 10, 12, 50, 14, 0, 14, 16, 0, 18,
            0, 18, 20, 50, 22, 22, 0, 22, 0, 26, 26, 0, 28, 50, 0, 35, 0, 50, 0] #defines the rent at each place
    price = [60, 0, 60, 200, 200, 100, 0, 100, 120, 0, 140, 150, 0, 140, 160, 200,
             180, 0, 180, 200, -50, 220, 0, 220, 240, 200, 260, 260, 0, 280, 50, 300,
             300, 0, 320, 200, 0, 350, 100, 400, -200] # defines the price at each place
    current_place = 0

    @staticmethod
    def get_place_name(place_num):
        return Space.place_names[place_num]

    @staticmethod
    def chance(current_place): # to check if you are at a chance space
        if current_place == Space.chance[0] or current_place == Space.chance[1] or current_place == Space.chance[2]:
            if Dice.roll(16) == 1: # goes to the space as defined on the chance card
                return 24
            elif Dice.roll(16) == 2:
                return 39
            elif Dice.roll(16) == 3:
    ...
    @staticmethod
    def community_chest(current_place): # to check is you are at a community chest space
        if current_place == Space.community_chest[0] or current_place == Space.community_chest[1] or current_place == \
            ...
            if Dice.roll(16) == 1: # goes to the space as defined on the community chest card
                return 0
            elif Dice.roll(16) == 2:
                return 30
            else:
